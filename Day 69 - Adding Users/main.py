...
class User(UserMixin, db.Model):
    ...
    def get_user_by_email(self, email):
        result = db.get_or_404(User, email)

        return result

    def look_for_duplicates(self, email):
...
def register():
    ...
    if form.validate_on_submit():
        hashed_password = werkzeug.security.generate_password_hash(form.password.data, method="pbkdf2:sha256",
                                                                   salt_length=10)
        new_user = User(
...
def login():
    ...
    if form.validate_on_submit():
        ...
        check_email = user_manager.get_user_by_email(email_login)

        if check_email and check_password_hash(check_email.password, password_login):
